 ...
 * 
 * 2.사용자가 구매를한다.
 * (조건:한 번에 한 상품 구매가능 수량은 여러개가능)
 * 구매할 제품번호와 수량을 입력받는다
 * 예를들면 1001 코카콜라 3개를 구매(개당 가격은 1000원)
 * 
 * 3.입력받은 값으로 tbl_product에서 cnt를 판매한 수만큼 뺀다
 * 예를 들면 코카콜라 cnt에서 -3을 한다(UPDATE)
 * 4.tbl_sale에 판매기록을 저장한다 (INSERT)
 * 예를들면 sno는 시퀀스값 , sname은 코카콜라,cnt는 3, tprice(3000 = 3 X 1000)
 * 
 * 선행조건) tbl_sale테이블을 생성한다
 * sno number PK,
