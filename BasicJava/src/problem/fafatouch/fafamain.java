...
public class fafamain {
	...
	public static void main(String[] args) {
		...
		// {"파파버거","새우버거","와퍼버거","싸이버거","빅맥버거"};
		// 메인메뉴를 저장하는 5칸짜립 배열 생성
		...
		while(true) {
			...
			// 출력!!
			System.out.println("▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒");
			System.out.println("▒ 고객님께서 주문하신");
			System.out.println("▒ 1)" + mainMenu[mainNum - 1][0] + ":" + mainMenu[mainNum - 1][1]); // 메인메뉴
			System.out.println("▒ 2)" + sideMenu[sideNum - 1][0] + ":" + sideMenu[sideNum - 1][1]); // 사이드메뉴
			System.out.println("▒ 3)" + drinkMenu[drinkNum - 1][0] + ":" + drinkMenu[drinkNum - 1][1]); // 음료메뉴
		...
		if (smallAns.equals("y")) {
			...
			while (true) {
				...
				if (totprice > money) {
					...
					// 지불금액 == 결제금액
					// 지불금액 > 결제금액(거스름돈 주고)
					System.out.println("▒결제완료! 고객님 맛있게드세요! ㅋㅅㅋ");
					break;
				}
			}
			...
			System.out.println("y 나 n을 입력해주세요");
			continue;
		}
